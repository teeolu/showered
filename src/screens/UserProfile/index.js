...
class UserProfile extends Component {
  static navigationOptions = ({ navigation }) => ({
    ...
    headerTitle: (
      <Block row middle><Text h4>User profile</Text></Block>
    )
  })
  ...
  renderHeading = () => {
    ...
    const { userdata: { lastName, firstName, email } } = this.props;
    return (
          ...
          <ImageBackground
            ...
            <View style={styles.userInfo}>
              ...
              <View>
                ...
                <Text h3 style={{ color: '#fff', fontSize: 14 }}>{email}</Text>
              </View>
            </View>
          </ImageBackground>
        </TouchableWithoutFeedback>
          ...
          <Card
            ...
            <Block row space="between" style={{ marginTop: 25 }}>
              <Block>
                ...
                <Block row center>
                  <Label blue />
                  <Text paragraph color="gray">Comfort</Text>
                </Block>
              </Block>
              <Block>
                ...
                <>
                  {(userMarketplaceData.map(item => (
                    <Block
                      ...
                      style={styles.driver}>
                      <TouchableOpacity
                        onPress={() => this.navigateToBrowse('CategoryDetails', { item })}>
                        <Block row center>
                          <View style={{ marginRight: 15 }}>
                            <Image
                              style={styles.avatar}
                              source={{ uri: item.uploadedImageArray[0].secureUrl }} />
                          </View>
                          <Block>
                            <Text h4>{item.marketPlaceName}</Text>
                            <Text paragraph color="gray">{item.email}</Text>
                          </Block>
                          <Block>
